routerApp.controller('customersCtrl', function($scope, $state, $window,$filter, $http, $interval, secService, $timeout, $cookieStore) {
    ...
    $interval(function(){
      console.log($scope.totalObj.sections[$scope.activeNav].time);
    ...
    $scope.pause = function() {
        $scope.pauseData = {
            ...
            'presentQuation': $scope.currentQua  ,
        ...
        var sendObj = {
            ...
            // "awsstatus": $scope.awsstatus1
        }
        sendObj['toalObj']['extradata'] = $scope.pauseData;
        console.log();
        sendObj.toalObj.sections[$scope.activeNav].time =  $scope.totalObj.sections[$scope.activeNav].time ;
    ...
    secService.sec().then(function(response) {
        ...
        console.log(response.data.sections[$scope.activeNav].time);
        //Show time start
    });
    ...
    $scope.getTestResult = function() {
        ...
        //Graph end
        //Save result
        $scope.awsstatus1="processing";
        console.log('save test ...');
        $scope.totalObj.awsstatus="proccess";
        var sendObj = {
            "useId": $cookieStore.get("loginAccess").id,
            "testId": Number($cookieStore.get("testId")),
            "toalObj": $scope.totalObj,
            ...
            "graphdata" : {
                "awsscore" : '' ,
                "verbal" : getGreScore.verbal,
                "quant" :  getGreScore.quant,
                "grescore" : getGreScore.greScore
                 }
        }
        console.log(sendObj.graphdata);
        sendObj['toalObj']['extradata'] = $scope.pauseData;
        secService.saveData(sendObj).then(function(res) {
            //   console.log(res);
