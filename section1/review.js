routerApp.controller('reviewCtrl', function($scope, $http, $state, $interval,$window, secService, $timeout, $cookieStore) {
    ...
    $scope.getTestResult = function() {
        ...
        var grapSections = [];
        var myChartData = [];
        var getGreScore={"quant":0 , "verbal":0, "greScore":0 ,"awa":0 };
        ...
        for(var i=2; i<6 ; i++){
            console.log(i,$scope.reasultPer.sections[i]) ;
            if($scope.reasultPer.sections[i].category == "quant"){
                getGreScore.quant = getGreScore.quant+$scope.reasultPer.sections[i].result ; 
            }else{
                getGreScore.verbal = getGreScore.verbal+$scope.reasultPer.sections[i].result ; 
            }
        }
        ...
        console.log($scope.totalObj);
        if($scope.totalObj.awsstatus !='proccess'){
           // $scope.totalGreScor.awa = $scope.totalObj.questions[0].options[0].score +  $scope.totalObj.questions[0].options[1].score ;
           ...
           $scope.totalGreScor.awa =Math.round(totalCountgetScr/2);
           console.log(totalCountgetScr);
        }
        $scope.getscoreDetails=function(){
            
            var x = $scope.totalGreScor.greScore;
            alert(x);
            switch (true) {
                
                case (x >= 260 && x <= 280):
            
                   $state.go('unsscore',{"id":12});
                    break;
                case (x >= 281 && x <= 300):
                $state.go('unsscore',{"id":11});
                    break;
                case (x >= 301 && x <= 320):
                $state.go('unsscore',{"id":10});
                    break;
                case (x >= 321 && x <= 340):
                $state.go('unsscore',{"id":9});
                    break;
               
            }
        }
        console.log(getGreScore, $scope.totalObj);
    ...
    $scope.showAns = function(obj) {
        console.log(obj);
        console.log(obj.question_type_id);quationType1Result
        if (obj.question_type_id != undefined) {
            ...
            if (obj.question_type_id == 6) {
                var result = quationType5Result(obj);
            }
            if (obj.question_type_id == 7) {
                var result = quationType5Result(obj);
            }
            return result;
        }
